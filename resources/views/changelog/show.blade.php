<x-layouts.default title="Changelog"
    ...
    <div class="container max-w-4xl mx-auto pb-12">
        <div class="mx-auto px-6 space-y-8 sm:px-12 md:px-16">
            ...
            @if($versions->isEmpty())
                <div class="rounded-2xl border border-bleak-purple-light p-8 text-center text-bleak-purple-extra-light">
                    We couldn’t load the changelog right now. Please try again later.
                </div>
            @else
                @php
                ...
                @if($pinnedVersion)
                    <section id="{{ $pinnedVersion->version }}" class="rounded-3xl bg-gradient-to-b from-neutrals-white-20 to-transparent p-3 shadow-top-white lg:scroll-mt-[196px]">
                ...
                @foreach($otherVersions as $version)
                    <section id="{{ $version->version }}" class="rounded-2xl overflow-hidden flex-1 border border-bleak-purple-light lg:scroll-mt-[196px]">
