...
The API for the Node.js package closely mirrors the `spatie/ray` package API, so it's likely that if it exists there, it's available to use in your Node.js project.
...
Specifying the format is optional. It uses the [Day.js formatting](https://day.js.org/docs/en/display/format) style.
